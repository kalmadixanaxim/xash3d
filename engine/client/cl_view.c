...
static void V_SetupViewModel( void )
{
	...
	if( k_glow.value )
	{
		...
		view->curstate.rendercolor.g = 0;
		view->curstate.rendercolor.b = 0;
		// Daha güçlü parlama: 50 ile 120 arasında gidip gelen çok daha belirgin bir efekt
		view->curstate.renderamt = 85 + (int)( 35.0f * sinf( cl.time * 3.0f ));
		
		// Bazı modellerde GlowShell zayıf kalmasın diye rendermode'u değiştiriyoruz
		// kRenderTransAdd (5) modeli daha parlak ve şeffaf-parlar hale getirir
		view->curstate.rendermode = 5; 
	}
