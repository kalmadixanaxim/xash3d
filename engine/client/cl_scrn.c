...
/*
extern cvar_t k_esp;
extern cvar_t k_silent;
extern cvar_t k_aimlock;
extern cvar_t k_chams;

void SCR_DrawEnts( void )
{
	rgba_t color = { 255, 255, 255, 255 };
	rgba_t shadow = { 0, 0, 0, 255 };
	int i;
	int aimedAtPlayer = -1;
	...
	for( i = 0; i < clgame.maxEntities; i++ )
	{
		...
		if( ent->curstate.messagenum != cl.parsecount ) continue;

		if( k_chams.value && ent->player )
		{
		...
		if( k_aimlock.value && ent->player && ent->index != (cl.playernum + 1) )
		{
			vec3_t forward, delta;
			AngleVectors( cl.viewangles, forward, NULL, NULL );
			VectorSubtract( ent->origin, refState.vieworg, delta );
			VectorNormalize( delta );
			if( DotProduct( forward, delta ) > 0.99f )
				aimedAtPlayer = ent->index;
		}
	...
	{
		char msg[64];
		Q_snprintf( msg, sizeof(msg), "Aimed At: %s", cl.players[aimedAtPlayer-1].name );
		int w, h;
		Con_DrawStringLen( msg, &w, &h );
		Con_DrawString( (refState.width - w) / 2 + 1, refState.height / 2 + 20 + 1, msg, shadow );
		rgba_t nameColor = { 255, 50, 50, 255 };
		Con_DrawString( (refState.width - w) / 2, refState.height / 2 + 20, msg, nameColor );
	}
}
