...
static qboolean CL_FindAimbotTarget( const vec3_t source, const vec3_t angles, vec3_t target_angles )
{
	...
	if( bestTarget )
	{
		static double next_aimbot_msg = 0.0;
		VectorSubtract( bestTargetPos, source, delta );
		VectorAngles( delta, target_angles );
		COM_NormalizeAngles( target_angles );
		
		if( host.realtime > next_aimbot_msg )
		{
			Con_Printf( "Aimbot: Locked on target %d\n", bestTarget->index );
			next_aimbot_msg = host.realtime + 1.0f;
		}
		return true;
	}
