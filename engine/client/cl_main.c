...
CVAR_DEFINE_AUTO( k_aimlock, "0", FCVAR_ARCHIVE, "enable aimlock" );
...
static qboolean CL_GetBoneTarget( cl_entity_t *ent, vec3_t target_pos )
{
	...
	// Find the head hitbox (typically group 1)
	pbbox = (mstudiobbox_t *)((byte *)hdr + hdr->hitboxindex);
	bone_idx = 0;
	for( i = 0; i < hdr->numhitboxes; i++ )
	{
		if( pbbox[i].group == 1 ) // Hitbox group 1 is usually head
		{
	...
	// Transform hitbox center for better head tracking
	vec3_t bone_center;
	VectorAdd(pbbox[i].bbmin, pbbox[i].bbmax, bone_center);
	VectorScale(bone_center, 0.5f, bone_center);

	// Transform bone_center by the world_bones matrix
...
static qboolean CL_FindAimbotTarget( const vec3_t source, const vec3_t angles, vec3_t target_angles )
{
	...
	for( i = 1; i <= cl.maxclients; i++ )
	{
		...
		// Assuming a travel time based on distance / speed (e.g., 2000 units/sec)
		float travel_time = VectorDistance( target_pos, source ) / 2000.0f;
...
static void CL_CreateCmd( void )
{
	...
	if( active && k_aimlock.value && CL_FindAimbotTarget( cl.refdef.vieworg, cmd->viewangles, target_angles ) )
	{
