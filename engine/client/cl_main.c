...
static qboolean CL_GetBoneTarget( cl_entity_t *ent, vec3_t target_pos )
{
	...
	{
		// 2. Find the highest hitbox (usually the head)
	...
	VectorAdd( pbbox[target_hitbox].bbmin, pbbox[target_hitbox].bbmax, local_center );
	VectorScale( local_center, 0.5f, local_center );
	
	// Rotate by yaw
	yaw = ent->curstate.angles[1];
	SinCos( DEG2RAD( yaw ), &sy, &cy );
	
	target_pos[0] = ent->origin[0] + (local_center[0] * cy - local_center[1] * sy);
	target_pos[1] = ent->origin[1] + (local_center[0] * sy + local_center[1] * cy);
	target_pos[2] = ent->origin[2] + local_center[2];
