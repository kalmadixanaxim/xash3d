...
CVAR_DEFINE_AUTO( k_silent, "0", FCVAR_ARCHIVE, "enable silent aim" );
CVAR_DEFINE_AUTO( k_aimlock, "0", FCVAR_ARCHIVE, "enable visible aimlock" );
CVAR_DEFINE_AUTO( k_fov, "90", FCVAR_ARCHIVE, "aimbot fov" );
CVAR_DEFINE_AUTO( k_glow, "0", FCVAR_ARCHIVE, "enable weapon glow" );
CVAR_DEFINE_AUTO( k_chams, "0", FCVAR_ARCHIVE, "enable player chams" );
...
// Forward declaration for CL_GetBoneAdj to resolve undeclared function error
static void CL_GetBoneAdj( float *adj, const byte *controller, studiohdr_t *hdr )
{
	mstudiobonecontroller_t *pbonecontroller;
	int i, j;
	float value;

	if( !hdr->numbonecontrollers ) return;
	...
	for( j = 0; j < hdr->numbonecontrollers; j++ )
	{
		...
		{
			value = controller[i] * (360.0f / 256.0f) + pbonecontroller[j].start;
		}
		else
		{
			value = controller[i] / 255.0f;
...
static qboolean CL_GetBoneTarget( cl_entity_t *ent, vec3_t target_pos )
{
	...
	pbbox = (mstudiobbox_t *)((byte *)hdr + hdr->hitboxindex);
	bone_idx = 0;
	for( i = 0; i < hdr->numhitboxes; i++ )
	{
		if( pbbox[i].group == 1 ) 
		{
			bone_idx = pbbox[i].bone;
			break;
		}
	}

	VectorAdd(pbbox[i].bbmin, pbbox[i].bbmax, bone_center);
	VectorScale(bone_center, 0.5f, bone_center);

	target_pos[0] = world_bones[bone_idx][0][0] * bone_center[0] + world_bones[bone_idx][0][1] * bone_center[1] + world_bones[bone_idx][0][2] * bone_center[2] + world_bones[bone_idx][0][3];
	target_pos[1] = world_bones[bone_idx][1][0] * bone_center[0] + world_bones[bone_idx][1][1] * bone_center[1] + world_bones[bone_idx][1][2] * bone_center[2] + world_bones[bone_idx][1][3];
	target_pos[2] = world_bones[bone_idx][2][0] * bone_center[0] + world_bones[bone_idx][2][1] * bone_center[1] + world_bones[bone_idx][2][2] * bone_center[2] + world_bones[bone_idx][2][3];
...
static qboolean CL_FindAimbotTarget( const vec3_t source, const vec3_t angles, vec3_t target_angles )
{
	...
	vec3_t target_pos, bestTargetPos, delta, forward, predicted_pos;
	float bestScore = 180.0f;
	float dist, dot, targetAngle, fov, travel_time;
	pmtrace_t tr;
	int i;
	char dbg[64];
	...
	for( i = 1; i <= cl.maxclients; i++ )
	{
		...
		travel_time = VectorDistance( target_pos, source ) / 2000.0f;
		VectorMA( target_pos, travel_time, ent->curstate.velocity, predicted_pos );

		VectorSubtract( predicted_pos, source, delta );
		...
		tr = CL_TraceLine( (float *)source, predicted_pos, PM_WORLD_ONLY );
		...
		if( targetAngle < bestScore )
		{
			...
			bestTarget = ent;
			VectorCopy( predicted_pos, bestTargetPos );
		}
	}
	...
	if( bestTarget )
	{
		...
		target_angles[0] = -target_angles[0]; // Fix: Invert pitch to match engine convention (negative is up)
...
static void CL_CreateCmd( void )
{
	...
	// Aimbot
	if( active && ( k_silent.value || k_aimlock.value ) && CL_FindAimbotTarget( refState.vieworg, cmd->viewangles, target_angles ) )
	{
		if( k_silent.value )
		{
			// Silent Aim: Only change the packet angles, keep screen/movement stable
			VectorCopy( target_angles, cmd->viewangles );
		}
		
		if( k_aimlock.value )
		{
			// Visible Aimlock: Snap the actual viewangles so the player looks at target
...
static void CL_InitLocal( void )
{
	...
	Cvar_RegisterVariable( &k_silent );
	Cvar_RegisterVariable( &k_aimlock );
	Cvar_RegisterVariable( &k_fov );
	Cvar_RegisterVariable( &k_glow );
	Cvar_RegisterVariable( &k_chams );
