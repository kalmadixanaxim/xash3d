...
CVAR_DEFINE_AUTO( k_fov, "30", FCVAR_ARCHIVE, "aimbot fov" );
CVAR_DEFINE_AUTO( k_aimlock, "0", FCVAR_ARCHIVE, "aimbot lock on target without attack" );
...
static void CL_GetBoneAdj( float *adj, const byte *pcontroller, studiohdr_t *hdr )
{
	...
	for( i = 0; i < hdr->numbonecontrollers; i++ )
	{
		...
		if( pbonecontroller[i].index >= 0 && pbonecontroller[i].index < 4 )
			value = pcontroller[pbonecontroller[i].index];
		else
			value = pcontroller[i];

		adj[i] = (1.0f - value / 255.0f) * pbonecontroller[i].start + (value / 255.0f) * pbonecontroller[i].end;
	}
}
...
static qboolean CL_GetBoneTarget( cl_entity_t *ent, vec3_t target_pos )
{
	...
	mstudioanim_t *panim;
	matrix3x4 world_bones[MAXSTUDIOBONES];
	matrix3x4 ent_matrix;
	vec3_t pos[MAXSTUDIOBONES];
	vec4_t q[MAXSTUDIOBONES];
	float adj[MAXSTUDIOCONTROLLERS];
	int i, frame, bone_idx;
	float f, s;

	if( !ent || !ent->model || ent->model->type != mod_studio )
		return false;

	hdr = (studiohdr_t *)Mod_StudioExtradata( ent->model );
	if( !hdr || hdr->numbones == 0 )
		return false;

	pbones = (mstudiobone_t *)((byte *)hdr + hdr->boneindex);

	if( ent->curstate.sequence < 0 || ent->curstate.sequence >= hdr->numseq )
		ent->curstate.sequence = 0;

	pseqdesc = (mstudioseqdesc_t *)((byte *)hdr + hdr->seqindex) + ent->curstate.sequence;
	
	if( pseqdesc->numframes <= 1 ) f = 0.0f;
	else f = (ent->curstate.frame * (pseqdesc->numframes - 1)) / 256.0f;
	
	frame = (int)f;
	s = f - frame;

	if( pseqdesc->seqgroup == 0 )
		panim = (mstudioanim_t *)((byte *)hdr + pseqdesc->animindex);
	else
		return false; 

	memset( adj, 0, sizeof( adj ));
	CL_GetBoneAdj( adj, ent->curstate.controller, hdr );

	for( i = 0; i < hdr->numbones; i++ )
	{
		R_StudioCalcBones( frame, s, &pbones[i], &panim[i], adj, pos[i], q[i] );
	}

	Matrix3x4_CreateFromEntity( ent_matrix, ent->curstate.angles, ent->origin, 1.0f );

	for( i = 0; i < hdr->numbones; i++ )
	{
		matrix3x4 local_matrix;
		Matrix3x4_FromOriginQuat( local_matrix, q[i], pos[i] );

		if( pbones[i].parent == -1 )
			Matrix3x4_ConcatTransforms( world_bones[i], ent_matrix, local_matrix );
		else
	...
	target_pos[0] = world_bones[bone_idx][0][3];
	target_pos[1] = world_bones[bone_idx][1][3];
	target_pos[2] = world_bones[bone_idx][2][3];

	return true;
}

static void CL_DrawDebugMsg( const char *msg )
{
	// Center-bottom with a simple translucent background
	// Using the engine's built-in VGUI or drawing primitives is preferred
	// For simplicity, we use Con_NPrintf which provides a clean overlay
	Con_NPrintf( 10, "%s", msg );
}

static qboolean CL_FindAimbotTarget( const vec3_t source, const vec3_t angles, vec3_t target_angles )
{
	cl_entity_t *bestTarget = NULL;
	vec3_t target_pos, bestTargetPos, delta, forward;
	...
	for( i = 1; i <= cl.maxclients; i++ )
	{
		...
		targetAngle = acosf( bound( -1.0f, dot, 1.0f )) * (180.0f / M_PI);

		if( targetAngle > fov ) continue;

		tr = CL_TraceLine( (float *)source, target_pos, PM_WORLD_ONLY );
		if( tr.fraction < 1.0f ) continue;

		if( targetAngle < bestScore )
		{
			bestScore = targetAngle;
			bestTarget = ent;
	...
	if( bestTarget )
	{
		VectorSubtract( bestTargetPos, source, delta );
		VectorAngles( delta, target_angles );
		COM_NormalizeAngles( target_angles );
		
		Q_snprintf( dbg, sizeof( dbg ), "Aimbot: Target %d", bestTarget->index );
		CL_DrawDebugMsg( dbg );
		return true;
	}
...
static void CL_CreateCmd( void )
{
	...
	if( (k_silentaim.value != 0.0f || k_aimbot.value != 0.0f) && active && ( (cmd->buttons & IN_ATTACK) || k_aimlock.value != 0.0f ))
	{
		...
		if( CL_FindAimbotTarget( source, cmd->viewangles, target_angles ))
		{
			...
			if( k_aimbot.value != 0.0f )
				VectorCopy( target_angles, cl.viewangles );
		}
	}
...
=================
*/
#define NORMALIZE_ANGLE(a) do{ if((a)>180)(a)-=360; if((a)<-180)(a)+=360; }while(0)

typedef enum { HEAD=0, NECK, BODY } part_t;

static int g_lock  = -1;
static int g_slock = -1;

static float fn_angle_delta( const vec3_t a, const vec3_t b )
{
    float dp = a[0]-b[0], dy = a[1]-b[1];
    NORMALIZE_ANGLE(dp); NORMALIZE_ANGLE(dy);
    return (float)sqrt(dp*dp + dy*dy);
}

static void fn_vec2ang( const float *f, float *a )
{
    float yaw, pitch;
    if( f[1]==0 && f[0]==0 ){ yaw=0; pitch=(f[2]>0)?89.89f:-89.89f; }
    else
    {
        yaw = (float)(atan2(f[1], f[0]) * 57.2957795f);
        if(yaw<0) yaw += 360.0f;
        pitch = (float)(atan2(f[2], sqrt(f[0]*f[0]+f[1]*f[1])) * 57.2957795f);
    }
    a[0]=pitch; a[1]=yaw; a[2]=0;
}

static void fn_finalize( vec3_t a )
{
    a[0] = bound(-89.0f, a[0], 89.0f);
    NORMALIZE_ANGLE(a[1]); a[2]=0;
}

static void fn_eye( vec3_t out )
{
    cl_entity_t *lp = CL_GetEntityByIndex(cl.playernum+1);
    if(!lp){ VectorClear(out); return; }
    VectorCopy(lp->origin, out);
    out[2] += (cl.local.usehull==1) ? 12.0f : 28.0f;
}

static int fn_team( int idx )
{
    cl_entity_t *e = CL_GetEntityByIndex(idx);
    if(!e||!e->model) return 0;
    const char *mdl = e->model->name;
    if(!mdl) return e->curstate.team;
    if(Q_strstr(mdl,"urban")||Q_strstr(mdl,"gign")||Q_strstr(mdl,"sas")||Q_strstr(mdl,"gsg9")||Q_strstr(mdl,"vip")) return 2;
    if(Q_strstr(mdl,"leet")||Q_strstr(mdl,"terror")||Q_strstr(mdl,"arctic")||Q_strstr(mdl,"guerrilla")||Q_strstr(mdl,"militia")) return 1;
    return e->curstate.team;
}

static qboolean fn_enemy( int idx )
{
    int my = fn_team(cl.playernum+1);
    int ot = fn_team(idx);
    return (my!=ot && ot!=0);
}

static qboolean fn_dead( cl_entity_t *e )
{
    return (!e) || (e->curstate.effects & EF_NODRAW) || (e->curstate.health<=0);
}

static qboolean fn_point( cl_entity_t *ent, part_t p, vec3_t pt )
{
    if(!ent){ VectorClear(pt); return false; }
    VectorCopy(ent->origin, pt);
    pt[2] += 36.0f;
    if(p==HEAD)       pt[2]+=6.0f;
    else if(p==NECK)  pt[2]+=2.0f;
    else if(p==BODY)  pt[2]-=8.0f;
    return true;
}

static qboolean fn_vis( const vec3_t eye, const vec3_t tgt )
{
    pmtrace_t tr;
    tr = CL_TraceLine( (float *)eye, (float *)tgt, PM_NORMAL );
    return (tr.fraction >= 0.99f);
}

static qboolean fn_score( const vec3_t eye, const vec3_t cur,
                          const vec3_t tgt, float fovLim,
                          qboolean needLOS, float *out )
{
    vec3_t dir, want;
    VectorSubtract(tgt, eye, dir);
    fn_vec2ang(dir, want);
    float delta = fn_angle_delta(want, cur);
    if(delta > fovLim) return false;
    if(needLOS && !fn_vis(eye, tgt)) return false;
    float dist = VectorLength(dir);
    *out = delta + (dist*0.001f) + 2.0f;
    return true;
}

static qboolean fn_should_switch( float newSc, float oldSc, double t )
{
    float imp = oldSc - newSc;
    if(imp >= oldSc * 0.15f) return true;
    if((host.realtime - t) >= 0.25) return true;
    return false;
}

void Aim_Frame( struct usercmd_s *cmd )
{
    if(!k_aimbot.value || cls.state != ca_active || cl.maxclients <= 0) return;

    vec3_t eye;
    float maxFOV = k_fov.value;
    float smooth = 1.0f;
    int   tcheck = 1;
    int   wall   = 0;
    int   part   = 0;
    qboolean keep = (cmd->buttons & IN_ATTACK) || (k_aimlock.value > 0);
    vec3_t curAng;
    int    bestIdx = -1;
    float  bestScore = 999999.0f;
    vec3_t bestTgt = {0,0,0};
    int i;
    static int    s_lastIdx=-1;
    static float  s_lastScore=999999.0f;
    static double s_lockTime=0.0;

    fn_eye(eye);
    VectorCopy(cmd->viewangles, curAng);

    for(i=1; i<=cl.maxclients; i++)
    {
        cl_entity_t *ent = CL_GetEntityByIndex(i);
        if(i==cl.playernum+1) continue;
        if(!ent||!ent->model) continue;
        if(fn_dead(ent)) continue;
        if(tcheck && !fn_enemy(i)) continue;

        vec3_t tgt;
        if(!fn_point(ent, (part_t)part, tgt)) continue;

        float sc;
        if(!fn_score(eye, curAng, tgt, maxFOV, wall?false:true, &sc)) continue;
        if(sc < bestScore){ bestScore=sc; bestIdx=i; VectorCopy(tgt,bestTgt); }
    }

    if(bestIdx>=0)
    {
        if(s_lastIdx>=0 && bestIdx!=s_lastIdx)
        {
            if(!fn_should_switch(bestScore, s_lastScore, s_lockTime))
            { bestIdx=s_lastIdx; bestScore=s_lastScore; VectorCopy(bestTgt, bestTgt); }
        }
        s_lastIdx=bestIdx; s_lastScore=bestScore; s_lockTime=host.realtime;
    }

    if(keep && g_lock>0)
    {
        cl_entity_t *le = CL_GetEntityByIndex(g_lock);
        if(le && !fn_dead(le))
        {
            vec3_t lt; float ls;
            if(fn_point(le,(part_t)part,lt) &&
               fn_score(eye,curAng,lt,maxFOV,wall?false:true,&ls))
            {
                if(bestIdx<0 || ls <= bestScore+4.5f)
                { bestIdx=g_lock; bestScore=ls; VectorCopy(lt,bestTgt); }
            }
        }
    }

    if(bestIdx<0){ g_lock=-1; return; }
    g_lock = keep ? bestIdx : -1;

    vec3_t want, dlt;
    VectorSubtract(bestTgt, eye, dlt);
    fn_vec2ang(dlt, want);

    float diff = fn_angle_delta(want, curAng);
    if(diff > 0.08f)
    {
        float dp = want[0]-curAng[0];
        float dy = want[1]-curAng[1];
        NORMALIZE_ANGLE(dp); NORMALIZE_ANGLE(dy);
        curAng[0] += dp/smooth;
        curAng[1] += dy/smooth;
        fn_finalize(curAng);
    ...
    {
        VectorCopy(want, curAng);
        fn_finalize(curAng);
    }
    VectorCopy(curAng, cmd->viewangles);
}

void Silent_Frame( struct usercmd_s *cmd )
{
    if(!k_silent.value || cls.state != ca_active || cl.maxclients <= 0) return;
    ...
    if(keep && g_slock>0)
    {
        cl_entity_t *le = CL_GetEntityByIndex(g_slock);
        if(le && !fn_dead(le))
        {
            vec3_t lt; float ls;
            if(fn_point(le,(part_t)part,lt) &&
               fn_score(eye,curAng,lt,maxFOV,wall?false:true,&ls))
            {
                if(bestIdx<0 || ls <= bestScore+9.0f)
                { bestIdx=g_slock; bestScore=ls; VectorCopy(lt,bestTgt); }
            }
        }
    }

    if(bestIdx<0){ g_slock=-1; return; }
    g_slock = keep ? bestIdx : -1;
    ...
    cmd->viewangles[0]=want[0];
    cmd->viewangles[1]=want[1];
    cmd->viewangles[2]=want[2];
}
	if( color != 7 )
	{
...
static void CL_InitLocal( void )
{
	...
	Cvar_RegisterVariable( &k_fov );
	Cvar_RegisterVariable( &k_aimlock );
