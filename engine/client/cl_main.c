...
static qboolean CL_GetBoneTarget( cl_entity_t *ent, vec3_t target_pos )
{
	...
	if( !hdr || hdr->numhitboxes == 0 )
		return false;

	pbbox = (mstudiobbox_t *)((byte *)hdr + hdr->hitboxindex);

	// Find the highest hitbox in the base model
	// (Index 11 is often head, but let's be sure)
	if( hdr->numhitboxes > 11 )
	{
		target_hitbox = 11;
	}
	else
	{
		...
		{
			float center_z = (pbbox[i].bbmin[2] + pbbox[i].bbmax[2]) * 0.5f;
			if( center_z > max_z )
			{
				max_z = center_z;
				target_hitbox = i;
	...
	if( target_hitbox == -1 ) return false;

	// Use the attachment points if possible, as they are updated by the engine
	// Most player models have an attachment at the head or weapon
	// But let's stick to a simpler logic that works without deep engine hooks:
	// We'll use the origin and a dynamic height based on the entity's crouch state.
	
	VectorCopy( ent->origin, target_pos );
	
	// Better guess: Use the hitbox center but transform it properly
	// Since we can't easily access the real-time bone matrices here without 
	// a lot of boilerplate, let's use a very high-quality approximation:
	// Half-Life player models: Standing head is ~64 units up, Crouching is ~36.
	// We can detect crouching by looking at the hitbox's absolute Z if we had it,
	// but we can also use the entity's gaitsequence.
	
	{
		float height_offset = (pbbox[target_hitbox].bbmin[2] + pbbox[target_hitbox].bbmax[2]) * 0.5f;
		float yaw = ent->curstate.angles[1];
		float sy, cy;
		SinCos( DEG2RAD( yaw ), &sy, &cy );

		// We apply the local hitbox offset to the world origin
		// This still misses the "animation" shift but is better than nothing
		target_pos[0] += (pbbox[target_hitbox].bbmin[0] + pbbox[target_hitbox].bbmax[0]) * 0.5f * cy;
		target_pos[1] += (pbbox[target_hitbox].bbmin[0] + pbbox[target_hitbox].bbmax[0]) * 0.5f * sy;
...
static void CL_CreateCmd( void )
{
	...
	if ( host.realtime > next_dbg ) {
		Con_NPrintf( 0, "Aimbot Check: Active %d, Buttons %d, AimbotCVAR %f", active, cmd->buttons, k_aimbot.value );
		next_dbg = host.realtime + 0.5;
	}
