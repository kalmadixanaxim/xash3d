...
static qboolean CL_GetBoneTarget( cl_entity_t *ent, vec3_t target_pos )
{
	...
	{
		if( pbbox[i].group == 1 ) // HITGROUP_HEAD
		{
			target_hitbox = i;
			break;
		}
	...
	// Fallback to index 11 if suggested by user and available
	if( target_hitbox == -1 && hdr->numhitboxes > 11 )
	{
		target_hitbox = 11;
	}

	// Fallback to chest (group 2)
	if( target_hitbox == -1 )
	{
		for( i = 0; i < hdr->numhitboxes; i++ )
		{
			if( pbbox[i].group == 2 ) // HITGROUP_CHEST
			{
				target_hitbox = i;
				break;
			}
	...
	VectorAdd( pbbox[target_hitbox].bbmin, pbbox[target_hitbox].bbmax, bone_origin );
	...
	target_pos[0] = ent->origin[0] + (bone_origin[0] * cy - bone_origin[1] * sy);
	target_pos[1] = ent->origin[1] + (bone_origin[0] * sy + bone_origin[1] * cy);
	target_pos[2] = ent->origin[2] + bone_origin[2];
...
static qboolean CL_FindAimbotTarget( const vec3_t source, const vec3_t angles, vec3_t target_angles )
{
	...
	for( i = 1; i <= cl.maxclients; i++ )
	{
		...
		tr = CL_TraceLine( (float *)source, target_pos, PM_STUDIO_BOX );
